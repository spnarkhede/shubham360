const generatePostmanCollectionPrompt = {
  emoji: '🎼',
  title: 'Generate Postman Collection',
  description: 'Create comprehensive Postman Collections with this AI prompt, including endpoint definitions, authentication flows, and test automation.',
  type: 'Prompts',
  category: 'Coding',
  tool: 'ChatGPT',
  output: 'Code',
...
Provide the complete Postman Collection in valid JSON format following the Collection v2.1.0 schema. Structure the output with proper indentation for readability. Include inline comments using the description fields to explain complex configurations. Format example responses with proper JSON structure and realistic data. Use consistent naming conventions throughout the collection.`,
  ...
  tips: [
    'Start by mapping out your API\'s architecture, identifying key endpoints and their relationships to create a logical folder hierarchy in Postman.',
    'Use environment variables to manage different stages of your API (dev, staging, production) and to handle authentication tokens dynamically, ensuring your collections remain flexible and reusable.',
    'Regularly update your Postman Collections with new endpoints and test scenarios to keep them relevant and aligned with your API\'s evolution, preventing integration issues.',
  ],
  howToUse: [
    'Inside #INFORMATION ABOUT ME section, fill in the [LIST YOUR API ENDPOINTS WITH METHODS], [SPECIFY AUTH TYPE: Bearer/API Key/OAuth/Basic], [YOUR API BASE URL], [LIST CUSTOM HEADERS IF ANY], and [SPECIFY VERSION IF APPLICABLE] placeholders with your specific API details.',
    'Example: "My API endpoints: GET /users, POST /users, DELETE /users/{id}. My authentication method: Bearer. My base URL: https://api.example.com. My required headers: Content-Type: application/json. My API version: v1."',
  ],
};

export default generatePostmanCollectionPrompt;
