const generatePhotorealHousePlanSheetsPrompt = {
  emoji: '🩻',
  title: 'Generate Photoreal House Plan Sheets',
  description: 'Generate photorealistic house renders with matching floor plans using this AI prompt, including exterior visualization, architectural drawings, and presentation layout.',
  type: 'Prompts',
  ...
  tips: [
    ...
    'Select key features that define your project\'s identity, like cantilevered upper volume, infinity-edge pool, or floor-to-ceiling pivot door, helping the AI image understand which architectural elements to emphasize in both render and floor plan.',
  ],
  howToUse: [
    'Fill in the [HOUSE STYLE], [SITE CONTEXT], [BUILDING SPEC] (levels, area, bed/bath), [MATERIAL PALETTE], and [KEY FEATURE 1], [KEY FEATURE 2], [KEY FEATURE 3] variables with your specific project details.',
    'Example: Create a single architectural presentation board image. TOP HALF: an ultra-photorealistic exterior visualization of a modern minimalist house with 2 levels, approx. 2,800 sq ft, 3 bed 2.5 bath, in a suburban hillside setting. Design language: rectilinear volumes with strong horizontal rooflines, deep window reveals, premium white stucco and natural wood accents. Include key features: infinity pool with glass edge, double-height living room glazing, cantilevered upper deck. Add a calm landscaped yard and a refined courtyard/pool composition if specified. Realistic dusk/golden-hour lighting with warm interior glow through large black-framed glazing; crisp shadows; correct scale; no distortion; buildable structure; detailed facade joints. BOTTOM HALF: a clean architectural floor plan drawing panel that matches the SAME building exactly. If 2 is two levels, show TWO floor plans side-by-side Upper Floor right. If one level, show one plan centered. Use black linework on white background with consistent stroke weights, doors with swing arcs, windows, walls thickness, stairs where needed. Label every room in English and show room sizes in sq ft. Keep the plan minimal and professional (like an architecture brochure). Ensure room arrangement is logically consistent with the exterior: double-height glazing corresponds to living room, courtyard/pool aligns with outdoor terrace, core placement matches. The whole output must look like a real architecture presentation sheet: clean white margins, subtle small captions only. Do NOT add random text, fake logos, or unreadable typography. Everything must be sharp and readable.',
  ],
};

export default generatePhotorealHousePlanSheetsPrompt;
