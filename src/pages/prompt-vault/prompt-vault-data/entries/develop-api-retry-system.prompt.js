const developAPIRetrySystemPrompt = {
  emoji: '🔄',
  title: 'Develop API Retry System',
  description: 'Implement robust API retry logic with this AI prompt, ensuring resilience against server failures and rate limit violations.',
  type: 'Prompts',
  category: 'Coding',
  tool: 'ChatGPT',
  output: 'Code',
...
Use clear variable names and modular functions that can be easily integrated into existing codebases.`,
  ...
  tips: [
    'Clearly define your API\'s rate limits and expected request volume to tailor the retry mechanism effectively, ensuring it aligns with your business needs and server constraints.',
    'Use monitoring tools to track retry metrics and identify patterns, which can help optimize backoff intervals and improve system resilience over time.',
    'Regularly review and update your retry logic to incorporate new resilience patterns and best practices, ensuring your system remains robust against evolving API constraints and network conditions.',
  ],
  howToUse: [
    'Inside #INFORMATION ABOUT ME section, fill in the [INSERT API ENDPOINT URL], [INSERT REQUESTS PER MINUTE], and [INSERT LANGUAGE/FRAMEWORK] placeholders with your specific API endpoint, expected request volume, and programming language or framework.',
    'Example: "My API endpoint is \'https://api.example.com/v1/resource\'. My expected request volume is 100 requests per minute. My programming language is Python using the Requests library."',
  ],
};

export default developAPIRetrySystemPrompt;
