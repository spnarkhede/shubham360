const setRequestTimeoutsPrompt = {
  emoji: '⏳',
  title: 'Set Request Timeouts',
  description: 'Implement robust timeout configurations with this AI prompt, addressing network failures and resource exhaustion in multi-layered systems.',
  type: 'Prompts',
  category: 'Coding',
  tool: 'ChatGPT',
  output: 'Code',
...
Provide the timeout implementation as structured code blocks with detailed comments explaining each configuration. Use markdown formatting with clear section headers for each timeout layer. Include configuration examples in multiple programming languages if applicable. Present error handling patterns as code snippets with inline explanations. Create a summary table showing recommended timeout values for different scenarios. Use bullet points for best practices and warnings. Include ASCII diagrams to illustrate timeout interactions across layers when helpful.`,
  ...
  tips: [
    'Analyze your network environment to tailor timeout settings that align with your application\'s specific needs and constraints.',
    'Implement monitoring tools to track timeout occurrences and adjust configurations dynamically based on performance metrics.',
    'Regularly test your timeout configurations under various failure scenarios to ensure robustness and prevent unexpected system behavior.',
  ],
  howToUse: [
    'Inside #INFORMATION ABOUT ME section, fill in the [INSERT CONNECTION TIMEOUT IN SECONDS], [INSERT READ TIMEOUT IN SECONDS], and [INSERT TOTAL TIMEOUT IN SECONDS] placeholders with your specific timeout requirements for each layer of your application.',
    'Example: If your connection timeout is "5 seconds", read timeout is "10 seconds", and total request timeout is "30 seconds", configure your system to handle these values effectively across all network operations.',
  ],
};

export default setRequestTimeoutsPrompt;
